...
def main( args ):
    ...
    agent = PGAgent(gym.make(args.env), DiscreteMLPPolicyValue, device=torch.device('cuda'))
    ...
    for episode in range(args.max_episode):
        ...
        if episode % args.interval == 0:
            print(f'Running reward at episode {episode}: {running_reward:.3f}; Length: {len(rollout):3d}')
